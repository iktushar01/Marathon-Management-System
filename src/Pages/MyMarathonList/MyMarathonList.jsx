...
const MyMarathonList = () => {
    ...
    const fetchMarathons = async () => {
      try {
        ...
        const response = await fetch(
          `https://stridez-server.vercel.app/MyMarathon/${user.email}`
        );
  ...
  const validateForm = () => {
    ...
    if (!formData.startRegDate)
      newErrors.startRegDate = "Start date is required";
    if (!formData.endRegDate) newErrors.endRegDate = "End date is required";
    if (!formData.marathonStartDate)
      newErrors.marathonStartDate = "Marathon date is required";

    if (
      formData.startRegDate &&
      formData.endRegDate &&
      formData.startRegDate > formData.endRegDate
    ) {
    ...
    if (
      ...
      formData.endRegDate &&
      formData.marathonStartDate < formData.endRegDate
    ) {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://stridez-server.vercel.app/marathons/${selectedMarathon._id}`,
        {
          method: "PUT",
          headers: {
  ...
  if (error) {
    return (
      <div className=" flex items-center justify-center">
        <Helmet>
          <title>My Marathon List | stridez</title>
        </Helmet>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                      ...
                      <Datepicker
                        ...
                        onChange={(date) =>
                          setFormData({ ...formData, startRegDate: date })
                        }
                        className={`w-full ${
                    ...
                    <div>
                      ...
                      <Datepicker
                        ...
                        onChange={(date) =>
                          setFormData({ ...formData, endRegDate: date })
                        }
                        className={`w-full ${
                    ...
                    <div>
                      ...
                      <Datepicker
                        ...
                        onChange={(date) =>
                          setFormData({ ...formData, marathonStartDate: date })
                        }
                        className={`w-full ${
            ...
            <div className="bg-white rounded-xl shadow-lg w-full max-w-md">
              <div className="p-6">
                ...
                <p className="mb-6 text-gray-600">
                  Are you sure you want to delete the marathon{" "}
                  <strong className="text-red-500">
                    "{selectedMarathon?.title}"
                  </strong>
                  ? This action will permanently remove all data associated with
                  this marathon.
                </p>
